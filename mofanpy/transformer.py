...
class PositionWiseFFN(keras.layers.Layer):
    def __init__(self, model_dim):
        ...
        diff = model_dim * 4
        self.l = keras.layers.Dense(diff, activation=keras.activations.relu)
...
class PositionEmbedding(keras.layers.Layer):
    def __init__(self, max_len, model_dim, n_vocab):
        ...
        pe = pos / np.power(1000, 2 * np.arange(model_dim)[None, :] / model_dim)
        pe[:, 0::2] = np.sin(pe[:, 0::2])
        pe[:, 1::2] = np.cos(pe[:, 1::2])
        pe = pe[None, :, :]  # [1, max_len, model_dim]
...
class Transformer(keras.Model):
    ...
    def translate(self, src, v2i, i2v):
        ...
        while True:
            ...
            decoded_z = self.decoder.call(
                y_embed, encoded_z, False,
                yz_look_ahead_mask=self._look_ahead_mask(y), xz_pad_mask=self._pad_mask(src_pad)
            )
    ...
    for t in range(step):
        ...
        if t % 50 == 0:
            ...
            print(f"step: {t}"
                  ...
                  f"| target: {''.join([data.i2v[i] for i in by[0, 1:10]])}"
                  f"| inference: {''.join([data.i2v[i] for i in np.argmax(logits, axis=1)[:10]])}")
            t0 = t1
...
def export_attention(model, data, name="transformer"):
    ...
    path = f"./visual/tmp/{name}_attention_matrix.pkl"
...
if __name__ == '__main__':
    ...
    print(f"Chinese time order: yy/mm/dd {d.date_cn[:3]} \nEnglish time order: dd/M/yyyy {d.date_en[:3]}")
    print(f"vocabularies: {d.vocab}")
